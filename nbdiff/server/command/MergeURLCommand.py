    ...
    def process(self, request, filename, db_session):
        ...
        try:
            ...
            if int(localFile.info()['Content-Length']) > max_size:
                errMsg = errMsg + "The Local notebook exceeds 20MB. Only notebooks below 20MB are accepted.<br/>"  
            if not ".ipynb" in localFile.info()['Content-Disposition']:
                errMsg = errMsg + "The Local notebook is not a .ipynb file. Only .ipynb files are accepted.<br/>"      
        except:
            errMsg = errMsg + "We are unable to access the Local notebook file from the given URL.<br/>"

        try:
            baseFile = urllib2.urlopen(baseURL)
            if int(baseFile.info()['Content-Length']) > max_size:
                errMsg = errMsg + "The Base notebook exceeds 20MB. Only notebooks below 20MB are accepted.<br/>"
            if not ".ipynb" in baseFile.info()['Content-Disposition']:
                errMsg = errMsg + "The Base notebook is not a .ipynb file. Only .ipynb files are accepted.<br/>"   
        except:
            errMsg = errMsg + "We are unable to access the Base notebook file from the given URL.<br/>"
         
        try:
            remoteFile = urllib2.urlopen(remoteURL)
            if int(remoteFile.info()['Content-Length']) > max_size:
                errMsg = errMsg + "The Remote notebook exceeds 20MB. Only notebooks below 20MB are accepted.<br/>"
            if not ".ipynb" in remoteFile.info()['Content-Disposition']:
                errMsg = errMsg + "The Remote notebook is not a .ipynb file. Only .ipynb files are accepted.<br/>" 
        except:
            errMsg = errMsg + "We are unable to access the Remote notebook file from the given URL.<br/>"               
            
        if len(errMsg) == 0:
            try:
                nb_local = parser.parse(localFile)
            except nbformat.NotJSONError:
                errMsg = errMsg + "The Local notebook contains invalid JSON data. <br/>"
            try:    
                nb_base = parser.parse(baseFile)
            except nbformat.NotJSONError:
                errMsg = errMsg + "The Base notebook contains invalid JSON data. <br/>"
            try:    
                nb_remote = parser.parse(remoteFile)
            except nbformat.NotJSONError:
                errMsg = errMsg + "The Remote notebook contains invalid JSON data. <br/>"
