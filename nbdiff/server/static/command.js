	...
	var undo = function(id) {
		if(typeof id != "undefined")
		{
			if(_undo_commands[id].length > 0) {
	            var command = _undo_commands[id].pop();
		...
		else
		{
			if(_undo_command_stack.length > 0) {
				var command = _undo_command_stack.pop();
...
MoveLeftCommand.prototype = {
    ...
    undo: function() {
    	var $button = $(this.merge_row._cells.base.element()).closest(".row").find('.row-cell-merge-controls-remote > input');
...
MoveRightCommand.prototype = {
    ...
    undo: function() {
    	var $button = $(this.merge_row._cells.base.element()).closest(".row").find('.row-cell-merge-controls-local > input');
